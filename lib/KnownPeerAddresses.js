...
const KnownPeerAddress = rfr('lib/KnownPeerAddress.js');
const PeerStatus = rfr('lib/PeerStatus.js');

class KnownPeerAddresses extends NetworkChildClass {
    constructor(params = {}) {
        super(params);
        ...
        if (params.localPeerAddress) {
            this._localPeerAddress = params.localPeerAddress;
        }
    }
    ...
    close(peerAddress) {
        if (!this.known(peerAddress)) {
            this.log('error', 'KnownPeerAddresses | Trying to close unknown peer', peerAddress);
            return false;
        }

        if (!this._map.get(peerAddress.id).hasPeerChannel()) {
            this.log('error', 'KnownPeerAddresses | Trying to close peer without channel', peerAddress);
            return false;            
        }
    ...
    setStatus(peerAddress, newStatus) {
        if (!this.known(peerAddress)) {
            this.log('error', 'KnownPeerAddresses | Trying yo set status of unknown peer', peerAddress);
            return false;
        }
        ...
        this.log('debug', 'Set status of: ' + peerAddress + ' to: ' + this._map.get(peerAddress.id).peerStatus);
